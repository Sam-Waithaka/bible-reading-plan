...
type ImageBlockProps = {
  asset?:
    | PublicWritingCard["og_image_detail"]
    | PublicResourceSeries["cover_image_detail"];
  className?: string;
  tone: string;
};
...
type BrowseSortOrder = "default" | "ascending" | "descending";
...
const sectionLabelClass =
  "text-xs font-black uppercase tracking-[0.18em] text-red-800 dark:text-red-200";
const heroLabelClass =
  "text-sm font-black uppercase tracking-[0.16em] text-red-700";
const centeredSectionHeaderClass =
  "shrink-0 text-center text-sm font-black uppercase tracking-[0.16em] text-zinc-950 dark:text-stone-100";

const countLabel = (count?: number) =>
  `${count ?? 0} ${(count ?? 0) === 1 ? "Article" : "Articles"}`;
const toneFor = (seed: number | string) =>
  fallbackTones[
    Math.abs(
      String(seed)
        .split("")
...
const ImageBlock = ({ asset, className = "", tone }: ImageBlockProps) => {
  ...
  return (
    <div
      className={`relative box-border w-full max-w-full min-w-0 overflow-hidden bg-gradient-to-br ${tone} ${className}`}
    >
      {responsiveAsset ? (
        <ResponsiveImage
          alt=""
...
const SkeletonBlock = ({ className = "" }: { className?: string }) => (
  <div
    className={`animate-pulse rounded-2xl border border-black/10 bg-white/70 shadow-lg shadow-zinc-900/5 dark:border-white/10 dark:bg-[#171717] ${className}`}
  />
);
...
const FeaturedArticleCard = ({
  ...
  article?: PublicWritingCard | null;
  eyebrow?: string;
  loading: boolean;
}) => {
      ...
      <div className="grid min-h-72 place-items-center rounded-3xl border border-black/10 bg-white/70 p-8 text-center shadow-2xl shadow-zinc-900/10 dark:border-white/10 dark:bg-zinc-950 dark:shadow-black/40 lg:min-h-[23rem]">
        <div>
          ...
          <h2 className="mt-4 font-serif text-3xl font-bold">
            No latest publication yet.
          </h2>
          <p className="mt-3 text-sm leading-6 text-zinc-600 dark:text-stone-400">
            Published writings will appear here once they are available.
          </p>
        </div>
      </div>
    );
...
const BrowseListCard = ({
  ...
  id?: string;
  items: BrowseItem[];
  tieBreaker: "default" | "label";
  title: string;
}) => {
  const [sortOrder, setSortOrder] = useState<BrowseSortOrder>("default");
  const sortedItems = useMemo(
    () =>
      [...items].sort((left, right) => {
        ...
        return tieBreaker === "default"
          ? left.defaultOrder - right.defaultOrder
          : left.label.localeCompare(right.label);
      }),
    [items, sortOrder, tieBreaker],
  );
  const isScrollable = sortedItems.length > 6;
  const sortButtonClass = (order: BrowseSortOrder) => {
    if (order === "default") {
      return `grid size-9 shrink-0 place-items-center rounded-full transition focus:outline-none focus:ring-2 focus:ring-red-700 ${
        sortOrder === "default"
          ? "text-red-800 dark:text-red-100"
          : "text-zinc-500 hover:text-red-800 dark:text-stone-400 dark:hover:text-red-100"
      }`;
    }

    return `grid size-9 shrink-0 place-items-center rounded-full border transition focus:outline-none focus:ring-2 focus:ring-red-700 ${
      sortOrder === order
        ? "border-red-800 bg-red-800 text-white shadow-sm shadow-red-950/20"
        : "border-black/10 bg-white text-zinc-600 hover:text-red-800 dark:border-white/10 dark:bg-[#171717] dark:text-stone-300 dark:hover:text-red-100"
    }`;
  };
  ...
  return (
    <section
      id={id}
      className="box-border w-full max-w-full min-w-0 scroll-mt-28"
    >
      <div className="mb-4 flex box-border w-full max-w-full min-w-0 flex-wrap items-center gap-3">
        <h2 className={`${sectionLabelClass} min-w-0 flex-1`}>{title}</h2>
        <div
          aria-label={`Sort ${title} by article count`}
          className="flex shrink-0 items-center gap-1.5"
          role="group"
        >
          <button
            aria-label={`${title}: ${defaultSortLabel}`}
            aria-pressed={sortOrder === "default"}
            className={sortButtonClass("default")}
            onClick={() => setSortOrder("default")}
            title={defaultSortLabel}
            type="button"
          >
            <ListRestart aria-hidden="true" size={16} />
          </button>
          <button
            aria-label={`${title}: sort by article count, ascending`}
            aria-pressed={sortOrder === "ascending"}
            className={sortButtonClass("ascending")}
            onClick={() => setSortOrder("ascending")}
            title="Fewest articles first"
            type="button"
          >
            <ArrowUpNarrowWide aria-hidden="true" size={16} />
          </button>
          <button
            aria-label={`${title}: sort by article count, descending`}
            aria-pressed={sortOrder === "descending"}
            className={sortButtonClass("descending")}
            onClick={() => setSortOrder("descending")}
            title="Most articles first"
            type="button"
          >
      ...
      {sortedItems.length ? (
        <div
          ...
          className={`rounded-2xl border border-black/10 bg-white shadow-lg shadow-zinc-900/5 dark:border-white/10 dark:bg-zinc-950 dark:shadow-black/40 ${isScrollable ? "max-h-72 overflow-y-auto overscroll-contain" : "overflow-hidden"}`}
          data-browse-list-scrollable={isScrollable ? "true" : "false"}
          ...
          {sortedItems.map((item) => {
            ...
            return (
              <a
                ...
                className="flex h-12 box-border w-full max-w-full min-w-0 items-center gap-3 border-b border-black/10 px-4 text-sm transition last:border-b-0 hover:bg-[#fffaf0] focus:outline-none focus:ring-2 focus:ring-inset focus:ring-red-700 dark:border-white/10 dark:hover:bg-[#171717]"
              >
                ...
                <span className="min-w-0 flex-1 truncate font-bold text-zinc-800 dark:text-stone-200">
                  {item.label}
                </span>
                <span className="shrink-0 text-xs font-semibold text-zinc-500 dark:text-stone-400">
...
const FeaturedCategoryCard = ({
  ...
  category: ResourcesHome["featured_categories"][number];
}) => (
  <a
    href={`/resources/category/${category.slug}`}
    className="group mb-5 flex min-h-72 box-border w-full max-w-full min-w-0 break-inside-avoid flex-col justify-between rounded-[1.75rem] border border-[#eaded0] bg-[#fffaf0] p-6 shadow-xl shadow-zinc-900/5 transition hover:-translate-y-1 hover:bg-white focus:outline-none focus:ring-2 focus:ring-red-700 dark:border-white/10 dark:bg-[#171717] dark:shadow-black/40 dark:hover:bg-[#171717]"
  >
    <span>
      <span className="inline-flex rounded-full border border-red-900/10 bg-red-50 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-red-800 dark:border-red-200/10 dark:bg-red-950/30 dark:text-red-100">
        Featured Collection
      </span>
      <span className="mt-8 block font-serif text-3xl font-semibold leading-tight tracking-normal text-zinc-950 dark:text-stone-100">
        {category.name}
      </span>
      <span className="mt-5 block h-px w-14 bg-red-700/70" aria-hidden="true" />
      <span className="mt-5 line-clamp-4 block text-sm leading-6 text-zinc-600 dark:text-stone-400">
        {category.description ||
          "Browse writings gathered around this collection."}
      </span>
    </span>
...
const FeaturedSeriesCard = ({ series }: { series: PublicResourceSeries }) => (
  <a
    ...
    className="group mb-5 grid min-h-72 box-border w-full max-w-full min-w-0 break-inside-avoid overflow-hidden rounded-[1.75rem] border border-black/10 bg-white shadow-xl shadow-zinc-900/5 transition hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-red-700 dark:border-white/10 dark:bg-zinc-950 dark:shadow-black/40 sm:grid-cols-[10rem_1fr]"
  >
    <ImageBlock
      asset={series.cover_image_detail}
      tone={toneFor(series.slug || series.id)}
      className="min-h-72"
    />
    <span className="flex min-w-0 flex-col justify-between p-6">
      <span>
        <span className="text-[11px] font-black uppercase tracking-[0.16em] text-red-800 dark:text-red-200">
          Featured Series
        </span>
        <span className="mt-5 block font-serif text-2xl font-semibold leading-tight tracking-normal text-zinc-950 dark:text-stone-100">
          {series.title}
        </span>
        <span className="mt-4 line-clamp-3 block text-sm leading-6 text-zinc-600 dark:text-stone-400">
          {series.description ||
            "Follow this curated journey through the church library."}
        </span>
      </span>
      <span className="mt-6 flex items-center justify-between gap-3 border-t border-black/10 pt-4 text-sm font-black text-zinc-600 dark:border-white/10 dark:text-stone-400">
        ...
        <ArrowRight
          size={15}
          className="text-red-800 transition group-hover:translate-x-1 dark:text-red-100"
          aria-hidden="true"
        />
      </span>
    </span>
...
const FeaturedShowcase = ({
  ...
  categories: ResourcesHome["featured_categories"];
  ...
  return (
    <div
      className="box-border w-full max-w-full min-w-0 rounded-[2rem] border border-[#eaded0] bg-[#fffaf0]/80 p-5 shadow-xl shadow-zinc-900/5 dark:border-white/10 dark:bg-zinc-950 dark:shadow-black/40 sm:p-6"
      data-resources-featured-shelf="true"
    >
      <div className="mb-5 grid min-w-0 gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-600 dark:text-stone-400">
            Editor-selected articles, collections, and series gathered for
            prominent discovery.
          </p>
        </div>
      ...
      >
        ...
        {series.map((item) => (
          <FeaturedSeriesCard series={item} key={`series-${item.id}`} />
        ))}
      </div>
    </div>
  );
...
const ResourceTypePreviewRail = ({
  rail,
}: {
  rail: PublicResourceTypeRail;
}) => {
  ...
  return (
    <article
      className="box-border w-full max-w-full min-w-0 rounded-[2rem] border border-[#eaded0] bg-[#fffaf0]/80 p-5 shadow-xl shadow-zinc-900/5 dark:border-white/10 dark:bg-zinc-950 dark:shadow-black/40 sm:p-6"
      data-resources-taxonomy-shelf="resource-type"
    >
      <div className="mb-5 grid min-w-0 gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
        ...
        >
          ...
          <span className="hidden min-w-0 max-w-[28rem] break-words text-left sm:inline">
      ...
      <ArticleShelf
        articles={rail.items ?? []}
        emptyText={`Published ${resourceType.name.toLowerCase()} resources will appear here soon.`}
      />
    </article>
  );
};
...
const CategoryPreviewRail = ({ rail }: { rail: PublicCategoryRail }) => {
  ...
  return (
    <article
      className="box-border w-full max-w-full min-w-0 rounded-[2rem] border border-[#eaded0] bg-[#fffaf0]/80 p-5 shadow-xl shadow-zinc-900/5 dark:border-white/10 dark:bg-zinc-950 dark:shadow-black/40 sm:p-6"
      data-resources-taxonomy-shelf="category"
    >
      <div className="mb-5 grid min-w-0 gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
        <div className="min-w-0">
          <p
      ...
      <ArticleShelf
        articles={rail.items ?? []}
        emptyText={`Published ${category.name.toLowerCase()} resources will appear here soon.`}
      />
    </article>
  );
};
...
const SeriesPreviewRail = ({ rail }: { rail: PublicSeriesRail }) => {
  ...
  return (
    <article
      className="box-border w-full max-w-full min-w-0 rounded-[2rem] border border-[#eaded0] bg-[#fffaf0]/80 p-5 shadow-xl shadow-zinc-900/5 dark:border-white/10 dark:bg-zinc-950 dark:shadow-black/40 sm:p-6"
      data-resources-taxonomy-shelf="series"
    >
      <div className="mb-5 grid min-w-0 gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
        <div className="min-w-0">
          <p
      ...
      <ArticleShelf
        articles={rail.items ?? []}
        emptyText={`Published writings from ${series.title} will appear here soon.`}
      />
    </article>
  );
};
  ...
  <section className="box-border w-full max-w-full min-w-0 rounded-2xl border border-black/10 bg-[#fffaf0] p-5 shadow-lg shadow-zinc-900/5 dark:border-white/10 dark:bg-zinc-950 dark:shadow-black/40">
    <div className="flex box-border w-full max-w-full min-w-0 flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex min-w-0 items-center gap-3 sm:gap-4">
        ...
        <div>
          <h2 className="text-base font-black text-zinc-950 dark:text-stone-100">
...
const ResourcesLanding = ({
  ...
  const resourceTypes = navigation?.resource_types.length
    ? navigation.resource_types
    : (home?.resource_types ?? []);
  ...
  const ministries = home?.ministries.length
    ? home.ministries
    : (navigation?.ministries ?? []);
  const scriptureItems = scriptureBooks.map(
    (book: PublicScriptureBook, index) => ({
      count: book.writing_count,
      defaultOrder: book.number ?? index + 1,
      href: `/resources/book/${encodeURIComponent(book.osis_id)}`,
      icon: ScriptureIcon,
      label: book.name,
    }),
  );
  const ministryItems = ministries.map(
    (ministry: PublicResourceMinistry, index) => ({
    ...
    >
        ...
        <ResourcesContainer className="xl:max-w-[1728px]">
          ...
          >
            <div className="w-full max-w-3xl min-w-0">
              ...
              <p className="mt-7 min-w-0 max-w-full break-words font-serif text-[clamp(2.75rem,7vw,4.75rem)] font-bold leading-[0.98] tracking-normal text-zinc-950 dark:text-stone-100 sm:mt-8 xl:text-[clamp(3.6rem,4vw,4.75rem)]">
                <span className="block min-w-0 break-words xl:whitespace-nowrap">
                  Study deeply.
                </span>
                <span className="block min-w-0 break-words xl:whitespace-nowrap">
                  Reflect faithfully.
                </span>
                <span className="block min-w-0 break-words text-red-700 xl:whitespace-nowrap">
                  Live differently.
                </span>
              </p>
              <p className="mt-7 min-w-0 max-w-[34rem] break-words text-base leading-7 text-zinc-700 dark:text-stone-300 sm:mt-8 sm:text-lg sm:leading-8">
                Articles, Bible studies, pastoral guidance, and devotional
                reflections.
              </p>
            ...
            <div
              className="box-border w-full max-w-full min-w-0 xl:justify-self-end xl:[&_a[data-resource-card-mode='editorial-cover-only']]:max-w-full"
              data-resources-hero-article="true"
            >
              <FeaturedArticleCard
                article={latestPublication}
                eyebrow="Latest Publication"
                loading={loading}
              />
            </div>
      ...
      <ResourcesContainer className="grid box-border w-full max-w-full min-w-0 gap-8 py-8 pb-32 sm:gap-10 sm:py-10 sm:pb-32 xl:pb-12">
        ...
        <section
          ...
          aria-labelledby="resources-latest"
        >
          <CenteredSectionHeader id="resources-latest" title="Latest" />
          <ArticleGrid
            articles={latestArticles}
            emptyText="Latest published writings will appear here soon."
            loading={loading}
          />
        </section>

        <section
          id="resources-resource-type-rails"
          className="box-border w-full max-w-full min-w-0 scroll-mt-28"
          aria-labelledby="resources-resource-type-rails-heading"
        >
          <CenteredSectionHeader
            id="resources-resource-type-rails-heading"
            title="Explore by Resource Type"
          />
          {loading ? (
            <div className="grid box-border w-full max-w-full min-w-0 gap-6">
              {[0, 1, 2].map((item) => (
                <SkeletonBlock key={item} className="h-56 rounded-[2rem]" />
              ))}
            </div>
          ) : resourceTypeRails.length ? (
            <div className="grid box-border w-full max-w-full min-w-0 gap-6">
              {resourceTypeRails.map((rail) => (
                <ResourceTypePreviewRail
                  key={rail.resource_type.id}
                  rail={rail}
                />
              ))}
            </div>
        ...
        <section
          id="resources-category-rails"
          className="box-border w-full max-w-full min-w-0 scroll-mt-28"
          aria-labelledby="resources-category-rails-heading"
        >
          <CenteredSectionHeader
            id="resources-category-rails-heading"
          ...
          {loading ? (
            ...
            <div className="grid box-border w-full max-w-full min-w-0 gap-6">
              {categoryRails.map((rail) => (
                <CategoryPreviewRail key={rail.category.id} rail={rail} />
              ))}
            </div>
          ) : null}
        </section>
        ...
        <section
          id="resources-series-rails"
          className="box-border w-full max-w-full min-w-0 scroll-mt-28"
          aria-labelledby="resources-series-rails-heading"
        >
          <CenteredSectionHeader
            id="resources-series-rails-heading"
            title="Explore by Series"
          />
          {loading ? (
            <div className="grid box-border w-full max-w-full min-w-0 gap-6">
              {[0, 1, 2].map((item) => (
            ...
            <div className="grid box-border w-full max-w-full min-w-0 gap-6">
              {seriesRails.map((rail) => (
                <SeriesPreviewRail key={rail.series.id} rail={rail} />
              ))}
            </div>
          ) : null}
        </section>
        ...
        {loading || scriptureItems.length || ministryItems.length ? (
          <section className="grid box-border w-full max-w-full min-w-0 gap-8 md:grid-cols-2">
            ...
            ) : scriptureItems.length ? (
              <BrowseListCard
                defaultSortLabel="Biblical order"
                emptyText=""
                id="resources-scripture"
            ...
            {loading ? (
              ...
              <BrowseListCard
                defaultSortLabel="Default ministry order"
                emptyText=""
                id="resources-ministry"
                items={ministryItems}
                tieBreaker="label"
                title="Browse Ministry"
              />
            ) : null}
          </section>
        ) : null}
